...
class mks_config:
    ...
    hash_bits = 256 (32 Bytes)
    mks_bits  = 32  (4 GB instance)
    disk_bits = 40  (1 TB storage)
    blk_bits  = 12  (4 KB block)
    """

    def __init__(self, shards=8, hash_bits=256, mks_bits=32, disk_bits=40, blk_bits=12):
        self.shards = shards
        ...
        self.mks_map_entry_sz = (self.shards * self.hash_sz) + (self.shards * self.mks_map_diskblk_sz)
        self.mks_map_sz = self.mks_blks * self.mks_map_entry_sz

        self.overhead = (float(self.mks_map_sz) / float(self.mks_sz)) * 100

    def get_overhead(self):
        return self.overhead

default = mks_config()
lower_ss = mks_config(shards = 4)
sha1 = mks_config(hash_bits = 160)
bigger_disk = mks_config(disk_bits = 43)

print "Default        :", str(default.get_overhead()), "%"
print "Less Shards (4):", str(lower_ss.get_overhead()), "%"
